...
class FishingGame:
    ...
    # 제목
        ...
        achievement_keys = list(self.achievements.keys())
    ...
    # 업적 카드
        for i, key in enumerate(achievement_keys):
            ...
            completed = ach["progress"] >= ach["goal"]

        # 희귀도 색
            rarity_color = RARITY_COLORS[ach["rarity"]]
        ...
        # 카드 색
            ...
            box_rect = pygame.Rect(x, y, 180, 120)
        ...
        # 진행도
            progress_text = font_tiny.render(
                f"{ach['progress']} / {ach['goal']}",
                True,
                YELLOW
            )

            screen.blit(progress_text, (x + 15, y + 45))

        # 보상
            reward_text = font_tiny.render(
                f"보상: {ach['reward']}G",
                True,
                WHITE
            )

            screen.blit(reward_text, (x + 15, y + 70))

        # 완료 여부
            if completed:

                done_text = font_tiny.render(
                    "COMPLETE",
                    True,
                    BRIGHT_GREEN
                )

                screen.blit(done_text, (x + 15, y + 95))

            else:

                locked_text = font_tiny.render(
                    "진행중...",
                    True,
                    GRAY
                )

                screen.blit(locked_text, (x + 15, y + 95))

    # 닫기 안내
        close_text = font_tiny.render(
            "[ESC] 닫기",
            True,
            GRAY
        )

        screen.blit(close_text, (20, SCREEN_HEIGHT - 30))
